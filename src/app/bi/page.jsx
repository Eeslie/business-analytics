...
// Count all available reports across all modules
const TOTAL_REPORTS = 6; // Sales Summary, Inventory Stock, Profit & Loss (Standard), Real-Time Report, Audit Trail, Financial Statement (Compliance)
const DATA_SOURCES = 3; // sales_orders, accounts_receivable, inventory_stocks
...
export default function BiOverviewPage() {
  ...
  // Fetch total orders from real-time API
  useEffect(() => {
    async function fetchTotalOrders() {
      try {
        ...
        setTotalOrders(data?.kpis?.orders ?? 0);
      } catch (err) {
  ...
  const STATS = [
    { label: "Total Reports", value: TOTAL_REPORTS.toString(), change: "", trend: "up" },
    { label: "Active Users", value: "1", change: "", trend: "up" },
    { label: "Data Sources", value: DATA_SOURCES.toString(), change: "", trend: "up" },
    { label: "Total Orders", value: loading ? "..." : formatNumber(totalOrders), change: "", trend: "up" }
  ];
